...
type ExportFormat = "css" | "scss" | "swift";

function generateSCSS(values: Record<string, string>, font: string): string {
  const defaults = getDefaults();
  ...
  for (const g of TOKEN_GROUPS) {
    for (const t of g.tokens) {
      const val = values[t.cssVar] ?? t.defaultVal;
      if (val === defaults[t.cssVar]) continue;
      const scssName = "$sitka" + t.cssVar.replace(/^--/, "-");
      if (t.type === "color") {
        scssVars.push(`${scssName}: ${val};`);
        rootLines.push(`  ${t.cssVar}: ${hexToTriple(val)};`);
      } else if (t.type === "full-color") {
        scssVars.push(`${scssName}: ${val};`);
        rootLines.push(`  ${t.cssVar}: #{${scssName}};`);
      } else if (t.type === "px") {
        scssVars.push(`${scssName}: ${val}px;`);
        rootLines.push(`  ${t.cssVar}: #{${scssName}};`);
      }
    }
  }
  ...
  if (scssVars.length === 0 && !fontLine) {
    return "// No changes from defaults — all tokens match the Sitka baseline.";
  }

  const varBlock = scssVars.length > 0 ? scssVars.join("\n") + "\n" : "";
  const rootBlock = rootLines.length > 0 ? `\n:root {\n${rootLines.join("\n")}\n}` : "";
  return `// Sitka Child Theme — generated by Theme Customizer\n\n${varBlock}${rootBlock}${fontLine}`.trim();
}

function generateSwift(values: Record<string, string>, font: string): string {
  const defaults = getDefaults();
  const colorLines: string[] = [];
  const radiusLines: string[] = [];

  const radiusSwiftName: Record<string, string> = {
    "--radius-sm": "sm",
    "--radius":    "base",
    "--radius-md": "md",
    "--radius-lg": "lg",
  ...
  function toCamel(cssVar: string): string {
    return cssVar.replace(/^--/, "").replace(/-([a-z])/g, (_, c) => c.toUpperCase());
  }
  ...
  for (const g of TOKEN_GROUPS) {
    for (const t of g.tokens) {
      const val = values[t.cssVar] ?? t.defaultVal;
      if (val === defaults[t.cssVar]) continue;
      if (t.type === "color" || t.type === "full-color") {
        const h = val.replace("#", "").padEnd(6, "0");
        const r = parseInt(h.slice(0, 2), 16) || 0;
        const gv = parseInt(h.slice(2, 4), 16) || 0;
        const b = parseInt(h.slice(4, 6), 16) || 0;
        colorLines.push(`        static let ${toCamel(t.cssVar)} = Color(red: ${r}/255, green: ${gv}/255, blue: ${b}/255)`);
      } else if (t.type === "px") {
        const name = radiusSwiftName[t.cssVar] ?? toCamel(t.cssVar);
        radiusLines.push(`        static let ${name}: CGFloat = ${val}`);
      }
    }
  }
  ...
  const fontOption = FONT_OPTIONS.find(f => f.value === font);
  const fontLine = fontOption?.stack
    ? `\n    struct Typography {\n        static let fontFamily = "${fontOption.stack.split(",")[0].replace(/'/g, "")}"\n    }`
    : "";

  if (colorLines.length === 0 && radiusLines.length === 0 && !fontLine) {
    return "// No changes from defaults — all tokens match the Sitka baseline.";
  }

  const colorBlock = colorLines.length > 0
    ? `\n    struct Colors {\n${colorLines.join("\n")}\n    }` : "";
  const radiusBlock = radiusLines.length > 0
    ? `\n    struct Radius {\n${radiusLines.join("\n")}\n    }` : "";

  return `// Sitka Child Theme — generated by Theme Customizer\n\nimport SwiftUI\n\nstruct SitkaTheme {${colorBlock}${radiusBlock}${fontLine}\n}`;
}
...
const FORMAT_TABS: { id: ExportFormat; label: string; ext: string }[] = [
...
function CodeExport({
  ...
  const hints: Record<ExportFormat, React.ReactNode> = {
    css: (
      <>
        Import after{" "}
        <code className="font-mono px-1 py-0.5 rounded-[var(--radius-sm)]" style={{ backgroundColor: "rgb(var(--surface-raised))" }}>
          sitka-tokens.css
        </code>{" "}
    ...
    scss: (
      <>
        Paste the <code className="font-mono px-1 py-0.5 rounded-[var(--radius-sm)]" style={{ backgroundColor: "rgb(var(--surface-raised))" }}>$sitka-*</code> variables before your Sitka import.
        The <code className="font-mono px-1 py-0.5 rounded-[var(--radius-sm)]" style={{ backgroundColor: "rgb(var(--surface-raised))" }}>:root</code> block applies the same overrides at runtime.
      </>
    ),
    swift: (
      <>
        Add <code className="font-mono px-1 py-0.5 rounded-[var(--radius-sm)]" style={{ backgroundColor: "rgb(var(--surface-raised))" }}>SitkaTheme</code> to your project and reference{" "}
        <code className="font-mono px-1 py-0.5 rounded-[var(--radius-sm)]" style={{ backgroundColor: "rgb(var(--surface-raised))" }}>SitkaTheme.Colors.accent</code> in SwiftUI views.
        Load custom fonts via <code className="font-mono px-1 py-0.5 rounded-[var(--radius-sm)]" style={{ backgroundColor: "rgb(var(--surface-raised))" }}>.font()</code> or{" "}
        <code className="font-mono px-1 py-0.5 rounded-[var(--radius-sm)]" style={{ backgroundColor: "rgb(var(--surface-raised))" }}>UIFont</code>.
      </>
    ),
  };

  const ext = FORMAT_TABS.find(t => t.id === format)?.ext ?? "css";
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-3">
        {/* Format tabs */}
        <div
          className="flex rounded-[var(--radius)] border overflow-hidden"
          style={{ borderColor: "rgb(var(--border))", backgroundColor: "rgb(var(--surface-raised))" }}
        >
          {FORMAT_TABS.map((tab, i) => (
            <button
              ...
              className="text-[12px] font-medium px-3 py-1.5 transition-standard"
              style={{
                backgroundColor: format === tab.id ? "rgb(var(--accent-subtle))" : "transparent",
                color: format === tab.id ? "rgb(var(--accent))" : "rgb(var(--text-tertiary))",
                borderRight: i < FORMAT_TABS.length - 1 ? "1px solid rgb(var(--border))" : "none",
              }}
            >
              {tab.label}
            </button>
...
export default function ThemeCustomizerPage() {
  ...
  const output =
    exportFormat === "scss"  ? generateSCSS(values, font) :
    exportFormat === "swift" ? generateSwift(values, font) :
                               generateCSS(values, font);

  const mimeType =
    exportFormat === "swift" ? "text/x-swift" :
    exportFormat === "scss"  ? "text/x-scss"  : "text/css";

  const ext = FORMAT_TABS.find(t => t.id === exportFormat)?.ext ?? "css";
  ...
  function handleDownload() {
    ...
    a.href = url;
    a.download = `sitka-child-theme.${ext}`;
    a.click();
      ...
      <div className="flex gap-8 items-start">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-8">
          ...
          <CodeExport
            output={output}
            format={exportFormat}
            onFormatChange={f => { setExportFormat(f); setCopied(false); }}
            onCopy={handleCopy}
